		...
		<movable-area class="area" :style="{ height: areaHeight }" @mouseenter="mouseenter" @mouseleave="mouseleave">
			<block v-for="(item, index) in imageList" :key="item.id">
				<movable-view class="view" :x="item.x" :y="item.y" direction="all" :damping="40"
					...
					<view class="area-con"
						...
						<view class="progress" :style="{ width: childWidth, height: childWidth}"
							v-show="item.progress>0&&item.progress<100">{{item.progress}}%</view>
			...
			<view class="add" v-if="imageList.length < number"
				...
				<view class="add-wrap" :style="{ width: childWidth, height: childWidth }">
					...
					height: 100%;" :draggable="false" src="../../assets/camera.png">
					</image>
	...
	export default {
		...
		methods: {
					...
					uni.chooseImage({
						...
						success: res => {
							...
							for (let i = 0; i < count; i++) {
								this.addProperties(res.tempFilePaths[i], undefined, res.tempFiles[i])
							}
							this.upload()
						}
					})
			...
			delImage(item, index) {
				...
				this.$emit("delImagefun", this.imageList)
				for (let obj of this.imageList) {
			...
			async upload() {
				let that = this;
				let promises = []
				this.imageList.forEach((img, index, arr) => {
					if (img.status == 0) {

						if (img.tempFiles.size <= this.configObj.maxSize) {
							~(async () => {


								console.log(999999, img)
								this.$emit('kaishi', true)
								img.status = await 1
								// await pathToBase64(img.src)
								// 	.then(path => {
								// 		img.src = path
								// 	})
								// 	.catch(error => {
								...
								let promise = await new Promise(function(resolve, reject) {
									let uploadTask = uploadFile(img, 'png', data => {
										resolve(data)
									}, reject, that.configObj)
									img.uploadTask = uploadTask
									uploadTask.onProgressUpdate(async function(res) {
										that.$set(img, 'progress', res.progress)
										img.uploadInfo = res
										// this.$emit("progressfun",res)
									});
								}).then(res => {
									this.$emit('kaishi', false)
									let url = res.data
									...
									img.status = 2
									img.url = url.data[0]
									this.$emit('successImg', this.imageList)
									...
									this.preStatus = true
									this.previewList = []
									console.log("erruploadFile", err)
									this.$emit('kaishi', false, true, err)

									img.status = 99
									// new Error('上传错误:' + err)
									// console.log(e)
									// console.log(
									// 	"Upload fail11111111111111111111111111111ed,Delete and try again!",
									// 	e)
									// uni.showToast({
									// 	title: "Upload failed,Delete and try again!",
									// 	icon: "none"
									// })
									// reject(e)
								})
							...
							this.previewList = []
							uni.showToast({
								title: "Failed: Maximum single support 4MB",
								icon: "none",
								duration: 3000
							})
							this.$emit('qingkong')
							new Error('Failed:Failed: Maximum single support 4MB')
				...
				Promise.all(promises)
					.then(res => {
						console.log("3", res)
						// console.log("Upload failed,Delete and try again!",res)
						// uni.hideLoading()
						this.sortList()
					}).catch(function(e) {
						console.log("3", res)
						// console.log("12313132",e)
						// uni.hideLoading()
					})
			},
			addProperties(item, status, tempFiles) {
				if (status === undefined) {
					status = 0
				}
